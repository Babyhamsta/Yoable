...
namespace Yoable.Managers
{
    ...
    public class YoloAI
    {
        ...
        // Default AI settings (can be overridden with ISettingsService)
        public float AIConfidence { get; set; } = 0.25f;
        public float ConsensusIoUThreshold { get; set; } = 0.5f;
        public int MinimumConsensus { get; set; } = 2;
        public bool UseWeightedAverage { get; set; } = true;
        public float EnsembleIoUThreshold { get; set; } = 0.5f;
        public bool UseGPU { get; set; } = false;

        // Legacy single model support
        public InferenceSession yoloSession => loadedModels.FirstOrDefault()?.Session;

        public YoloAI(IDialogService? dialogService = null, ISettingsService? settingsService = null)
        {
            ...
            {
                AIConfidence = _settingsService.GetSetting("AIConfidence", 0.25f);
                ConsensusIoUThreshold = _settingsService.GetSetting("ConsensusIoUThreshold", 0.5f);
                MinimumConsensus = _settingsService.GetSetting("MinimumConsensus", 2);
                UseWeightedAverage = _settingsService.GetSetting("UseWeightedAverage", true);
                EnsembleIoUThreshold = _settingsService.GetSetting("EnsembleIoUThreshold", 0.5f);
                UseGPU = _settingsService.GetSetting("UseGPU", false);
            }
        }
        ...
        private List<YoloDetection> ApplyEnsembleConsensus(List<YoloDetection> allDetections)
        {
            ...
            float consensusIoUThreshold = ConsensusIoUThreshold;
            int minimumConsensus = MinimumConsensus;
            bool useWeightedAverage = UseWeightedAverage;
            float confidenceThreshold = AIConfidence;
            ...
            float ensembleIoUThreshold = EnsembleIoUThreshold;
        ...
        // TODO: Implement in Phase 3 UI migration
        ...
        public void LoadModelFromPath(string modelPath)
        {
            try
            {
                ...
                if (!TryInitializeSession(model, UseGPU, out string errorMessage))
                {
