...
/// <summary>
/// Cross-platform LabelManager for managing YOLO label data
/// </summary>
public class LabelManager
{
    ...
    private int LoadYoloLabelsOptimized(string labelFile, ImageManager imageManager)
    {
        ...
        foreach (var ext in possibleExtensions)
        {
            ...
            if (imageManager.ImagePathMap.TryGetValue(potentialImageFile, out var foundInfo))
            {
                imageInfo = foundInfo;
            ...
            foreach (var line in File.ReadLines(labelFile))
            {
                ...
                try
                {
                    ...
                    var label = new LabelData($"Imported Label {existingCount + newLabels.Count + 1}",
                        new LabelRect(x, y, width, height));
                    newLabels.Add(label);
                    labelsAdded++;
                }
    ...
    public int LoadYoloLabels(string labelFile, string imagePath, ImageManager imageManager)
    {
        ...
        try
        {
            // Use cached dimensions from ImageManager instead of opening bitmap
            int imgWidth = (int)imageInfo.OriginalDimensions.Width;
            int imgHeight = (int)imageInfo.OriginalDimensions.Height;
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                try
                {
                    ...
                    var label = new LabelData($"Imported Label {labelCount}", new LabelRect(x, y, width, height));
    ...
    // Direct port of ExportLabelsToYolo - now uses cached dimensions
    public void ExportLabelsToYolo(string filePath, string imagePath, List<LabelData> labelsToExport)
    {
        // This method is typically called with cached ImageInfo available
        // For now, we'll read dimensions using our image service
        // In practice, the caller should pass dimensions from ImageManager
        ...
        // Note: imageWidth and imageHeight should be passed by caller from ImageManager
        // For backward compatibility, we'll extract from the first label's context
        // This is a workaround - ideally caller should pass dimensions

        // Assuming caller has ImageManager context, we'll need dimensions passed
        // For now, throw if called without proper setup
        throw new InvalidOperationException(
            "ExportLabelsToYolo requires image dimensions. Use ExportLabelsToYoloWithDimensions instead.");
    }

    /// <summary>
    /// Export labels with explicit dimensions (preferred method)
    /// </summary>
    public void ExportLabelsToYoloWithDimensions(string filePath, int imageWidth, int imageHeight, List<LabelData> labelsToExport)
    {
        using StreamWriter writer = new(filePath)
        {
            AutoFlush = true
    ...
    // For AI labels - using LabelRect instead of System.Drawing.Rectangle
    public void AddAILabels(string fileName, List<LabelRect> detectedBoxes)
    {
        if (!labelStorage.ContainsKey(fileName))
            labelStorage.TryAdd(fileName, new List<LabelData>());
        ...
        foreach (var box in detectedBoxes)
        {
            ...
            var labelCount = labelStorage[fileName].Count + 1;
            var label = new LabelData($"AI Label {labelCount}", box);
